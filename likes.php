...
<body>
    ...
    <section class="liked-videos">
        ...
        <h1 class="heading">Video favorite</h1>

        <div class="box-container">

            <?php
            $user_class = $_COOKIE['user_class'] ?? '';

            $select_likes = $conn->prepare("SELECT l.* FROM likes l
                                             JOIN content c ON l.content_id = c.id
                                             WHERE l.user_id = ? 
                                             AND (c.class = ? OR c.class IS NULL)");
            $select_likes->execute([$user_id, $user_class]);
