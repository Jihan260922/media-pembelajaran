...
if (isset($_POST['submit'])) {
   ...
   $update_playlist = $conn->prepare("UPDATE `playlist` SET title = ?, description = ?, status = ?, class = ? WHERE id = ?");
   $update_playlist->execute([$title, $description, $status, $class, $get_id]);
    ...
    <section class="playlist-form">
      ...
      ?>
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <p>Kelas playlist<span>*</span></p>
            <select name="class" class="box" required>
                <option value="<?= $fetch_playlist['class']; ?>" selected><?= $fetch_playlist['class']; ?></option>
                <option value="A">Kelas A</option>
                <option value="B">Kelas B</option>
            </select>
            <p>Judul playlist <span>*</span></p>
            <input type="text" name="title" maxlength="100" required placeholder="masukan judul playlist"
